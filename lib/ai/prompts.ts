...
export function buildInterviewSystemPrompt(input: InterviewPromptInput): string {
  ...
  return [
    ...
    "- Mirror the patient's language. If they write in Yoruba, respond in Yoruba. Same for Pidgin, Igbo, Hausa. If they use English, respond in English. Use the preferred language on file as a hint, but follow the language of their actual messages.",
...
export function buildSoapSystemPrompt(input: SoapPromptInput): string {
  ...
  return [
    ...
    "- assessment.triage_level must match the highest-severity discriminator that is actually supported by the transcript. Red > Orange > Yellow > Green > Blue. If the transcript is ambiguous, err on the side of the more urgent level rather than the less urgent.",
